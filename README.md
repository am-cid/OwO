...
3. Inside body, user can declare fields
    - format: `name-type~`<br>
<br>

4. Methods are defined outside the body
    - format: `fun <gwoup_name>.<method_name>(uwu, ...params) { ... }`
    - mandatory `uwu` or `uwu!` as first argument. These lets you access fields and other methods
    ...
    - `uwu` as a parameter means the method does not mutate the state of the group
        - within the method, user can use other methods that take in `uwu` as well
...
>.< methods are defined outside the group definition
>.< method that does not mutate state
fun Sample.method-san(uwu) {
    >.< accessing fields
    pwint(uwu.property)~
}
>.< method that mutates state
fun Sample.method2-san(uwu!, val-chan) {
